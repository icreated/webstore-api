...
import co.icreated.wstore.bean.UserCredentials;
...
public class AuthenticationEndpoints {
  ...
  public Response authenticateUser(@Parameter(description = "Login Form",
      schema = @Schema(implementation = UserCredentials.class),
      required = true) UserCredentials credentials) {

    IdempiereUserService userService = new IdempiereUserService();

    try {
      SessionUser sessionUser =
          authenticate(userService, credentials.getUsername(), credentials.getPassword());
  ...
  private SessionUser authenticate(IdempiereUserService userService, String username,
