...
import co.icreated.wstore.bean.PaymentParam;
...
public class CheckoutEndpoints {
  ...
  public Response createOrder(@Parameter(description = "Order Form",
      schema = @Schema(implementation = Order.class), required = true) Order order,
  ...
  @Operation(summary = "Void order", description = "Void order",
      responses = {@ApiResponse(responseCode = "500", description = "Order not processed"),
          @ApiResponse(responseCode = "403", description = "Order forbidden"),
          @ApiResponse(responseCode = "200", description = "Order processed"),})
  public Response voidOrder(@Parameter(description = "Order Form",
      schema = @Schema(implementation = Order.class), required = true) Order order,
      @Context OrderService orderService) {
  ...
  public Response payment(
      @Parameter(description = "Payment Param",
          schema = @Schema(implementation = PaymentParam.class),
          required = true) PaymentParam paymentParam,
      @Context PaymentService paymentService, @Context OrderService orderService) {
